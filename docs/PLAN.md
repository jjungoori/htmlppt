...
| M23 | 접근성·터치·성능 | ARIA/키보드 내비, 터치 제스처(이동/핀치 리사이즈/회전), 대량 객체 렌더 성능 | 🟡 진행중 — 터치 슬라이스 코어 완료: `core/gesture.ts` 순수 read-only(DOM 미접근·결정론적). `gestureFromPointers(s0,s1,c0,c1)`: 두 손가락 시작/현재 샘플 → `{pan(센트로이드 이동), scale(거리비), rotate(±180 최단경로 트위스트), pivot(시작 센트로이드)}` 분해, 손가락 동좌표 시작 시 scale=1·rotate=0로 graceful degrade. `applyGesture(o,g)`: 객체 center를 pivot 기준 scale→rotate→pan 후 `{x,y,angle,scaleX,scaleY}` 패치 반환(w/h 불변, 크기는 scale에 — transform 모델 일관). 브라우저 터치 핸들러가 패치를 `store.patch`로 커맨드 레이어 경유(단일 undo). 10테스트(전체 252 통과). **a11y 슬라이스 코어 완료**: `core/a11y.ts` 순수 read-only — `objectAriaLabel`(html 태그/엔티티 제거→레이블, 빈 객체는 placeholder-aware 폴백·80자 절단), `objectAriaAttrs`(role/roledescription·tabindex·aria-selected, locked 반영), `tabOrder`(zIndex 무관 읽기순=상→하,좌→우), `tabNavigate`(랩어라운드 Tab/Shift+Tab), `spatialNavigate`(현재 객체 center 기준 방향 90° 콘 내 최근접, 축정렬 가중·엣지 무랩). 12테스트(전체 264 통과). **성능 슬라이스 코어 완료**: `core/culling.ts` 순수 read-only(DOM 미접근·결정론적) — 뷰포트 컬링으로 화면에 겹치는 객체만 DOM에 마운트해 대량 덱 응답성 유지. `cullObjects(objs,viewport,margin?)` 선형 패스(회전 반영 `aabb` ∩ 뷰포트, 입력순 보존, margin halo로 스크롤 오버렌더) + `buildSpatialGrid(objs,cellSize)`/`queryGrid` 균일 그리드(객체 AABB를 cell 버킷에 등록, 뷰포트가 덮는 cell만 방문→정밀 AABB 확인, 다중 cell 객체 de-dup, 입력순 재방출)로 대량 덱 쿼리 sublinear. 둘 다 같은 결과(선형≡그리드 랜덤 산포 검증). 8테스트(전체 274 통과). 브라우저 배선(렌더러 ARIA 주입·키 핸들러→store.setSelection, 뷰포트→queryGrid 마운트)은 후속 |
| M24 | 제품화 | README + API 문서 + 사용 예제, npm 패키징/배포 메타, 데모 사이트 | 🟡 진행중 — 패키징/문서 슬라이스 완료: `README.md`(quickstart·EditorOptions 표·`exportHTML`↔`importDeckDocument` 라운드트립 예제·public API 표면 요약·dev 스크립트), `LICENSE`(MIT), `package.json` 배포 메타 하드닝(`exports` 맵 types+import 조건부, `sideEffects:false` 트리셰이킹, `files`에 README/LICENSE 동봉, `keywords`/`homepage`/`repository`/`bugs`/`engines>=18`). tsc·vitest(274) 무회귀. 데모 사이트 배선은 후속 |
