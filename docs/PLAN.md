...
| M23 | 접근성·터치·성능 | ARIA/키보드 내비, 터치 제스처(이동/핀치 리사이즈/회전), 대량 객체 렌더 성능 | 🟡 진행중 — 터치 슬라이스 코어 완료: `core/gesture.ts` 순수 read-only(DOM 미접근·결정론적). `gestureFromPointers(s0,s1,c0,c1)`: 두 손가락 시작/현재 샘플 → `{pan(센트로이드 이동), scale(거리비), rotate(±180 최단경로 트위스트), pivot(시작 센트로이드)}` 분해, 손가락 동좌표 시작 시 scale=1·rotate=0로 graceful degrade. `applyGesture(o,g)`: 객체 center를 pivot 기준 scale→rotate→pan 후 `{x,y,angle,scaleX,scaleY}` 패치 반환(w/h 불변, 크기는 scale에 — transform 모델 일관). 브라우저 터치 핸들러가 패치를 `store.patch`로 커맨드 레이어 경유(단일 undo). 10테스트(전체 252 통과). ARIA/키보드·성능·브라우저 터치 배선은 후속 |
